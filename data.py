...
def addData(areaData, theme, item):
    new_data = createNewData(item)

    theme_data = areaData.get(theme, {})
    ...
    areaData[theme] = theme_data

    return areaData

result = {}
area_code = { '1': '서울특별시', '2': '인천광역시', '3': '대전광역시', '4': '대구광역시', '5': '광주광역시', '6': '부산광역시', '7': '울산광역시', '8': '세종특별자치시', '31': '경기도', '32': '강원도', '33': '충청북도', '34': '충청남도', '35': '경상북도', '36': '경상남도', '37': '전락북도', '38': '전라남도', '39': '제주특별자치도'}
...
necessary_keys = ['contentid', 'addr1', 'firstimage', "firstimage2", "mapx", "mapy", 'title']
...
for item in data['response']['body']['items']['item']:
    ...
    temp_list = area_data.get('temp', [])
    temp_list.append(item)

    area_data['temp'] = temp_list
...
for area in result['area']:
    ...
    for index in selected:
        ...
        if item['cat2'] in theme_list:
            ...
            result['area'][area] = newData
        if item['cat3'] in theme_list:
            newData = addData(result['area'][area], theme_code[theme_list[item['cat3']]], item)
            result['area'][area] = newData
        if item['cat1'] == 'A05':
            newData = addData(result['area'][area], theme_code['11'], item)
            result['area'][area] = newData
        else:
            newData = addData(result['area'][area], theme_code['10'], item)
            result['area'][area] = newData
